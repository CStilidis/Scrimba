...
function pxToRem(px) {
  return (
    px / parseInt(getComputedStyle(document.body).getPropertyValue("font-size"))
  );
}
  ...
  function addNoDelay(content) {
    ...
    circle.HTML.onclick = () => {
      ...
      anime({
        ...
        duration: 200,
        easing: "linear",
      }).finished.then(() => removeElement(circle, circles));
  ...
  function resolveCollision() {
      ...
      for (const otherCircle of circles) {
        if (circle !== otherCircle && circle.collideTo(otherCircle)) {
          ...
          if (!circle.movable) {
            ...
            otherCircle.xCenter +=
              (2 * distanceOverlap * (circle.xCenter - otherCircle.xCenter)) /
              distance;
            otherCircle.yCenter +=
              (2 * distanceOverlap * (circle.yCenter - otherCircle.yCenter)) /
              distance;
            ...
            circle.xCenter -=
              (distanceOverlap * (circle.xCenter - otherCircle.xCenter)) /
              distance;
            circle.yCenter -=
              (distanceOverlap * (circle.yCenter - otherCircle.yCenter)) /
              distance;
    ...
    if (enableDynamicCollision) {
      for (const collisionPair of collisionPairs) {
        ...
        const m1 =
          (dpNorm1 * (c1.radius - c2.radius) + 2 * c2.radius * dpNorm2) /
          (c1.radius + c2.radius);
        const m2 =
          (dpNorm2 * (c2.radius - c1.radius) + 2 * c1.radius * dpNorm1) /
          (c1.radius + c2.radius);
